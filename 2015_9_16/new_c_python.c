...
/*
 *  create and enable an iscsi block target
 *
 *  execute sequence:
...
int creat_block_target(storage_obj_t *sobj, iscsitarget_t *target)
{
    ...
    int step_1 = 0, step_2 = 0, step_3 = 0, step_4 = 0 ,step_5 = 0;
    ...
    sobj->plugin = PyInt_AsLong(pRetVal);
    Py_DECREF(pArgs);
    Py_DECREF(pRetVal);

    printf("return 2 for create_block_backstore sucess : %d\n", sobj->plugin);
    step_1 = sobj->plugin;
    ...
    if (step_1 > 0)
    {
        ...
        pRetVal = PyObject_CallObject(pFunc, pArgs);
        printf("return 0 for map sucess : %d\n", PyInt_AsLong(pRetVal));
        ...
        if (!(step_2||step_3||step_4||step_5))
        {
...
/*
 *  create and enable an iscsi fileio target
 *
 *  execute sequence:
 *  1)create_block_backstore
 *  2)creat_target
...
int creat_fileio_target(storage_obj_t *sobj, iscsitarget_t *target)
{
    ...
    /*create fileio bacstore*/
    pFunc = PyDict_GetItemString(pDict,"create_fileio_backstore");
    if ( !pFunc || !PyCallable_Check(pFunc) )  
    {
        printf("can't find  create_fileio_backstore");  
        getchar();  
        return -1;  
    }
    pArgs = PyTuple_New(3);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("s",sobj->dev));
    PyTuple_SetItem(pArgs, 1, Py_BuildValue("s",sobj->name));
    PyTuple_SetItem(pArgs, 2, Py_BuildValue("i",sobj->size));
    pRetVal = PyObject_CallObject(pFunc, pArgs);
    sobj->plugin = PyInt_AsLong(pRetVal);
    Py_DECREF(pArgs);
    Py_DECREF(pRetVal);

    printf("return 1 for create_fileio_backstore sucess : %d\n", sobj->plugin);
    step_1 = sobj->plugin;
    ...
    if (step_1 > 0)
    {
        ...
        if ( !pFunc || !PyCallable_Check(pFunc) )  
        {
            printf("can't find set_tpg_attr");  
            getchar();  
            return -1;  
        }
        pArgs = PyTuple_New(10);
        ...
        if (!(step_2||step_3||step_4||step_5))
        {
            printf("creat_fileio_target %s success\n",sobj->wwn);
        
        }
        else
        {
            printf("create target failed so delete the backstore and target\n");
            delete_backstore(sobj->plugin, sobj->name);
            delete_target(sobj->wwn);
        }
    }
    else
 ...
 * @return 0 success; -1 otherwise
 */
int delete_backstore(int plugin, char name[])
{
    PyObject *pName, *pModule, *pDict, *pFunc, *pArgs, *pRetVal;
