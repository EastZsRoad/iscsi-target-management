...
static int iscsi_del_backstore(int iscsi_plugin, char name[]);

 
#define __DEBUG__ //you could turn off the debug infomation by comment out this line
#ifdef __DEBUG__
...
int iscsi_create_target(storage_obj_t *sobj, iscsitarget_t *target)
{
    ...
    int step_1 = -1, step_2 = -1, step_3 = -1, step_4 = -1,step_5 = -1,step_6 = -1, step_7 = 0;
    ...
    {
        dprintf("can't find create_backstore");  
        getchar();  
        return -1;  
    }
    pArgs = PyTuple_New(4);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("i",sobj->iscsi_plugin));
    ...
    dprintf("return 0 for step_1 success : %d\n", step_1);
    ...
    if (step_1 == 0)
    {
        ...
        Py_DECREF(pArgs);
        Py_DECREF(pRetVal);
        dprintf("return 0 for step_2 success : %d\n", step_2);
        if (step_2 == 0)
        {
            /*create tpg*/
            ...
            Py_DECREF(pArgs);
            Py_DECREF(pRetVal);
            dprintf("return 0 for step_3 success : %d\n", step_3);
            ...
            PyTuple_SetItem(pArgs, 0, Py_BuildValue("s",target->wwn));
            PyTuple_SetItem(pArgs, 1, Py_BuildValue("i",target->tag));
            PyTuple_SetItem(pArgs, 2, Py_BuildValue("i",sobj->iscsi_plugin));
            PyTuple_SetItem(pArgs, 3, Py_BuildValue("s",sobj->name));
            pRetVal = PyObject_CallObject(pFunc, pArgs);
            step_4 = PyInt_AsLong(pRetVal);
            Py_DECREF(pArgs);
            Py_DECREF(pRetVal);
            dprintf("return 0 for step_4 success : %d\n",step_4);
            ...
            Py_DECREF(pArgs);
            Py_DECREF(pRetVal);
            dprintf("return 0 for step_5 success : %d\n", step_5);
            ...
            Py_DECREF(pArgs);
            Py_DECREF(pRetVal);
            dprintf("return 0 for step_6 sucess : %d\n", step_6);
            ...
            {
                printf("No ACLs list set\n");
            }

            if(target->attrib->authentication == true)
            {
                if (target->attrib->generate_node_acls == true)//use tpg setting
                {
                    iscsi_set_mutual_auth(target->wwn, target->tag, target->chap_mutual_userid, target->chap_mutual_password,target->chap_userid, target->chap_password);
                    printf ("set_TPG_mutual_auth success\n");
                }
                else//use acl setting
                {
                    ...
                    printf ("set_acl_mutual_auth success\n");
                }
            }else 
            {
                printf("No authentication set\n");
            }
            ...
            else
            {
                ...
                iscsi_del_backstore(sobj->iscsi_plugin, sobj->name);
                iscsi_del_target(0,target->wwn );
            }
        }
        else
        {
...
int iscsi_del_all_targets(void)
{
    ...
    dprintf("return 0 for delete success : %d\n", result); 
...
int iscsi_del_target(bool del_back, char wwn[])
{
    ...
    pArgs = PyTuple_New(2);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("i",del_back));
    PyTuple_SetItem(pArgs, 1, Py_BuildValue("s",wwn));
    pRetVal = PyObject_CallObject(pFunc, pArgs);
    result = PyInt_AsLong(pRetVal);
    dprintf("return 0 for delete the target success : %d\n", result);
...
static int iscsi_del_backstore(int iscsi_plugin, char name[])
{
    ...
    pArgs = PyTuple_New(2);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("i",iscsi_plugin));
    PyTuple_SetItem(pArgs, 1, Py_BuildValue("s",name));
    pRetVal = PyObject_CallObject(pFunc, pArgs);
    result = PyInt_AsLong(pRetVal);
    dprintf("return 0 for delete the delete_backstore success : %d\n", result);
...
/*
 * Set acls mutual authentication credential for an iscsi target
 * @wwn     iscsi target's wwn
 * @tag     iscsi tpg tag index default = 1
 * @acl_wwn iscsi acls initiator existed list (a list which you wish the initiator in could connected in target)
...
int iscsi_set_ACLs_auth(char wwn[], int tag, char acl_wwn[][MAXNAMELEN], int number, char mutual_user[], char mutual_passwd[],char user[], char passwd[])
{
    ...
    if (number > 0)
    {
        for (i = 0; i < number; ++i)
        {
            ...
            PyTuple_SetItem(pArgs, 3, Py_BuildValue("s",user));
            PyTuple_SetItem(pArgs, 4, Py_BuildValue("s",passwd));
            PyTuple_SetItem(pArgs, 5, Py_BuildValue("s",mutual_user));
            PyTuple_SetItem(pArgs, 6, Py_BuildValue("s",mutual_passwd));
            pRetVal = PyObject_CallObject(pFunc, pArgs);
            result = PyInt_AsLong(pRetVal);
            Py_DECREF(pArgs);
            Py_DECREF(pRetVal);
            dprintf("return 0 for set_ACLs_mutual_auth success : %d\n", result);
        }
    
    }
    else
    {
        printf("set ACLs auth number should greater than zero\n");
    }
...
int iscsi_del_ACLs_auth(char wwn[], int tag)
{
    ...
    dprintf("return 0 for del_ACLs_mutual_auth success : %d\n", result);
...
int iscsi_set_mutual_auth(char wwn[], int tag, char mutual_user[], char mutual_passwd[],char user[], char passwd[])
{
    ...
    pRetVal = PyObject_CallObject(pFunc, pArgs);
    result = PyInt_AsLong(pRetVal);
    dprintf("return 0 for set_TPG_mutual_auth success : %d\n", result);
...
int iscsi_del_mutual_auth(char wwn[], int tag)
{
    ...
    pRetVal = PyObject_CallObject(pFunc, pArgs);
    result = PyInt_AsLong(pRetVal);
    dprintf("return 1 for set_TPG_mutual_auth success : %d\n", result);
...
int iscsi_set_initiators(char wwn[], int tag, char node_acls[][MAXNAMELEN], int number)
{
    ...
    {
        printf("set initiators number should greater than zero\n");
    }
...
int iscsi_del_initiators(char wwn[], int tag, char node_acls[][MAXNAMELEN], int number)
{
    ...
    {
        printf("delete initiators number should greater than zero\n");
    }
