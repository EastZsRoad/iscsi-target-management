...
typedef struct attributes
{
    ...
    bool generate_node_acls;// if generate_node_acls = 1 use tpg setting and if generate_node_acls = 0 use acls setting
...
typedef struct iscsitarget
{
    ...
    parameters_t    *parame;
    portals_t       *portal;
    char chap_mutual_password[MAXCHAPPWD];  //Attention : password length to be at least 12 bytes and no more than 16 bytes!
    char chap_mutual_userid[MAXCHAPUSER];   // "mutual-rts-user",
    char chap_password[MAXCHAPPWD];         //Attention : password length to be at least 12 bytes and no more than 16 bytes!
